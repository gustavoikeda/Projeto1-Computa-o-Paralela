...
void processar_linha(char *linha, Sensor sensores[]) {
    ...
    if (sscanf(linha, "%19s %19s %19s %19s %lf status %9s",
               sensor, data, hora, tipo, &valor, status) != 6)
        return;
...
int main(int argc, char *argv[]) {
    ...
    while (fgets(buffer, sizeof(buffer), arquivo)) {
        ...
        if (!linhas[total_linhas]) { printf("Erro de alocação\n"); return 1; }
        total_linhas++;
    }
    fclose(arquivo);
    ...
    if (!threads || !chunk) { printf("Erro de alocação\n"); return 1; }
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        chunk[i].fim      = (i == num_threads - 1) ? (int)total_linhas : (i + 1) * bloco;
    ...
    double tempo_gasto = (ts_fim.tv_sec  - ts_inicio.tv_sec) +
    ...
    for (int i = 0; i < MAX_SENSORES; i++) {
        if (sensores[i].count_temperatura > 1) {
            double media     = sensores[i].temperatura_soma / sensores[i].count_temperatura;
            double variancia = (sensores[i].soma_quadrados_temperatura /
                                sensores[i].count_temperatura) - (media * media);
